...
import { VSyncTTS } from '../mods/vsync-tts';
...
const ChatArea: React.FC<ChatAreaProps> = React.memo(({
    ...
    const vsyncRef = useRef<VSyncTTS | null>(null);
    ...
    useEffect(() => {
        ...
        const FLOATING_CODE_CONFIG = {
            INITIAL_COUNT: 15,
            STAGGER_DELAY_MS: 400,
        ...
        const colors = isOracleMode
            ? ['#00ff00', '#00cc00', '#00aa00']
            : ['#00d4ff', '#ff00ff', '#00ff88', '#ff6b6b', '#ffd700'];

        const createFloatingCode = () => {
            const code = document.createElement('div');
            code.style.position = 'absolute';
            code.style.fontFamily = "'Fira Code', 'Consolas', monospace";
            code.style.fontSize = `${FLOATING_CODE_CONFIG.MIN_FONT_SIZE + Math.random() * FLOATING_CODE_CONFIG.FONT_SIZE_RANGE}px`;
            ...
            code.style.opacity = `${FLOATING_CODE_CONFIG.MIN_OPACITY + Math.random() * FLOATING_CODE_CONFIG.OPACITY_RANGE}`;
            code.style.animation = `floatHorizontal ${FLOATING_CODE_CONFIG.MIN_DURATION_S + Math.random() * FLOATING_CODE_CONFIG.DURATION_RANGE_S}s linear infinite`;

            container.appendChild(code);

            setTimeout(() => {
                code.remove();
            }, FLOATING_CODE_CONFIG.LIFETIME_MS);
        };

        // Create initial batch
        for (let i = 0; i < FLOATING_CODE_CONFIG.INITIAL_COUNT; i++) {
            setTimeout(createFloatingCode, i * FLOATING_CODE_CONFIG.STAGGER_DELAY_MS);
        }

        const interval = setInterval(createFloatingCode, FLOATING_CODE_CONFIG.SPAWN_INTERVAL_MS);
    ...
    // Initialize VSyncTTS for Hanna
    useEffect(() => {
        if (activeAgent.id === AgentId.CREATIVE) {
            vsyncRef.current = new VSyncTTS({
                agentId: 'hanna',
                enableAnimation: false, // Will enable when we have viseme images
                ttsProvider: 'web-speech' // Using free Web Speech for now
            });
            console.log('[ChatArea] VSyncTTS initialized for Hanna');
        }
    }, [activeAgent.id]);

    // Auto-play AI responses for Hanna
    useEffect(() => {
        if (!voiceEnabled || !vsyncRef.current) return;
        ...
        if (lastMessage && lastMessage.role === Role.MODEL && lastMessage.text) {
            console.log('[ChatArea] Speaking:', lastMessage.text.substring(0, 50) + '...');
            vsyncRef.current.speak(lastMessage.text).catch(err => {
                console.error('[ChatArea] TTS error:', err);
            });
        }
