...
const HOOKS = [
  { id: 'gemini-2.0-flash-exp', name: 'Hyper-Flash', desc: 'Fast, Standard Intelligence', color: 'text-blue-400' },
  { id: 'gemini-1.5-pro', name: 'DeepLogic Pro', desc: 'Advanced Reasoning & Code', color: 'text-amber-400' },
  { id: 'gemini-2.0-flash-exp', name: 'Nano-Banana', desc: 'Visual Generation Capable', color: 'text-pink-400' }
];
...
export const Labs: React.FC<LabsProps> = ({ agents, onCreateAgent, initialAgentId }) => {
  ...
  useEffect(() => {
    if (selectedHook.id === 'gemini-1.5-pro') setIgorComment("Yesss! The Big Brain! DeepLogic Pro is powerful!");
    else if (selectedHook.id === 'gemini-2.0-flash-exp') setIgorComment("Nano-Banana! It will have eyes to see!");
  }, [selectedHook]);
